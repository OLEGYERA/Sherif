                    ...
                    <form role="form"
                        ...
                        <div class="panel-body">
                            ...
                            @foreach($dataTypeRows as $row)
                                ...
                                @endif
                                @if($row->field == 'slug')
                                @php $row->display_name = 'SEO URL' @endphp
                                @endif
                                ...
                                <div class="form-group col-md-{{ $display_options->width or 12 }}" @if(isset($display_options->id)){{ "id=$display_options->id" }}@endif">
                                    <label for="parent_id">Выберите родительскую категорию:</label>
                                    <select class="form-control" name='parent_id'>
                                    <option value=''{{ $dataTypeContent->parent_id == NULL ? 'selected' : ''}}></option>
                                        @foreach($categories as $category)
                                            <option value='{{$category->id}}'{{ $dataTypeContent->parent_id == $category->id ? 'selected' : ''}}>{{$category->name}}, уровень: {{$category->depth}}</option>
                                        @endforeach
                                    </select>
                                </div>
                                @endif
